...
async function endpoint(request: FastifyRequest<{Querystring: LocationExportQuery}>, reply: FastifyReply) {
    ...
    return await sql`COPY (SELECT time, id, trip_id, TRUNC(delay_min::NUMERIC, 3) delay_min, TRUNC(latitude::NUMERIC, 7) latitude, TRUNC(longitude::NUMERIC, 7) longitude, TRUNC(speed::NUMERIC, 3) speed, recorded_timestamp, next_stop_id 
